...
const Logo: React.FC<{ size?: number; className?: string }> = ({ size = 36, className = "" }) => (
  ...
  >
    <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-[0_0_15px_rgba(139,92,246,0.6)] overflow-visible">
      ...
      <path d="M44,38 L62,50 L44,62 Z" fill="currentColor" className="opacity-90 text-white" />
    </svg>
  </div>
);
...
const NotificationsPanel: React.FC<{ 
  ...
  <>
    ...
    <aside 
      className={`fixed top-0 right-0 bottom-0 z-[1100] w-full sm:w-96 bg-[#0a0c12] border-l border-white/10 transition-transform duration-500 shadow-2xl flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
    >
...
const App: React.FC = () => {
  ...
  if (isBooting) {
    return (
      <div className="fixed inset-0 z-[2000] bg-[#01040a] flex flex-col items-center justify-center p-6 transition-colors duration-700">
        <Logo size={100} className="mb-12 animate-pulse text-violet-500" />
        <div className="space-y-6 text-center">
          <h1 className="text-3xl font-orbitron font-black tracking-[0.5em] text-white">EPITRACK</h1>
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col selection:bg-violet-500/40">
      <header className="sticky top-0 z-[100] border-b border-white/5 bg-[#01040a]/80 backdrop-blur-xl transition-colors duration-500">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          >
            ...
            <div className="hidden lg:block">
              <h1 className="text-lg font-orbitron font-black tracking-[0.3em] text-white leading-none">EPITRACK</h1>
          ...
          <div className="flex-1 max-w-xl mx-12 hidden md:flex items-center glass rounded-2xl px-5 py-3 border border-white/5 focus-within:border-violet-500/40 focus-within:ring-4 focus-within:ring-violet-500/10 transition-all">
            ...
            <input 
              ...
              className="bg-transparent border-none focus:ring-0 text-sm ml-4 w-full text-slate-100 placeholder:text-slate-700 font-medium outline-none" 
            />
            {searchQuery && (
          ...
          <nav className="flex items-center gap-4 sm:gap-6">
            ...
            <button 
              ...
              className="flex items-center gap-3 px-4 sm:px-6 py-3 bg-violet-600 hover:bg-violet-500 text-white rounded-xl font-bold transition-all text-[10px] sm:text-xs uppercase tracking-widest font-orbitron shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:scale-105"
            >
              <Plus size={18} />
        ...
        {view === 'series-detail' && selectedSeries && (
          <div className="space-y-10">
            <button 
              ...
              className="flex items-center gap-4 text-slate-500 hover:text-white transition-colors font-bold text-[10px] sm:text-xs uppercase tracking-[0.3em] group"
            >
      ...
      <footer className="border-t border-white/5 py-16 bg-black/40 mt-24">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-10 opacity-40">
          <div className="flex items-center gap-6">
            ...
            <div>
              <span className="font-orbitron font-bold text-sm tracking-[0.5em] block text-white uppercase">EPITRACK</span>
              <span className="text-[10px] font-mono block text-slate-500 mt-1">Universal Tracking Core X-9</span>
            </div>
          </div>
          <div className="flex flex-wrap justify-center gap-6 sm:gap-10 text-[10px] font-bold uppercase tracking-[0.4em] font-mono text-slate-400">
