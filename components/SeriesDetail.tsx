...
const SeriesDetail: React.FC<SeriesDetailProps> = ({ series, onUpdate }) => {
  ...
  const handleToggleComplete = (episodeId: string) => {
    ...
    // Track the latest completed episode if it was just marked
  ...
  return (
    <div className="relative min-h-screen pb-20">
      <div className="w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 xl:gap-10">
          <div className="lg:col-span-8 space-y-6 xl:space-y-8">
            <div className="aspect-video bg-black rounded-[1.5rem] xl:rounded-[2rem] overflow-hidden border border-white/5 light:border-slate-200 shadow-3xl">
              {activeEpisode ? (
                <VideoPlayer 
              ...
              <div className="p-6 xl:p-10 glass rounded-[1.5rem] xl:rounded-[2.5rem] relative overflow-hidden group">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
                  <div className="flex items-center gap-4 xl:gap-8">
                    <div className="w-14 h-14 xl:w-16 xl:h-16 glass rounded-2xl flex-shrink-0 flex items-center justify-center text-violet-500 font-mono font-bold text-2xl xl:text-3xl shadow-xl border border-white/10 light:border-slate-200">
                      {activeEpisode.number}
                    </div>
                    <div className="min-w-0">
                      <h2 className="text-xl xl:text-3xl font-extrabold text-white light:text-slate-900 tracking-tight leading-tight line-clamp-2 group-hover:text-violet-400 transition-colors">
                        {activeEpisode.title}
                      </h2>
                ...
                <div className="p-5 xl:p-6 bg-white/[0.02] light:bg-slate-50 border border-white/5 light:border-slate-200 rounded-2xl">
                  <p className="text-slate-400 light:text-slate-600 text-sm xl:text-base leading-relaxed font-medium">
          ...
          <div className="lg:col-span-4 space-y-6 xl:space-y-8">
            <div className="p-8 xl:p-10 glass rounded-[1.5rem] xl:rounded-[2.5rem] sticky top-24 overflow-hidden shadow-3xl">
              <div className="relative aspect-square rounded-3xl overflow-hidden mb-8 border border-white/10 light:border-slate-200 shadow-inner">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#01040a] light:from-white/40 to-transparent opacity-80" />
              </div>
              
              <h1 className="text-2xl xl:text-3xl font-black text-white light:text-slate-900 mb-6 font-orbitron uppercase tracking-tight leading-none line-clamp-2">{series.title}</h1>
              
              <div className="flex flex-wrap gap-3 mb-10">
                <span className="px-3 py-1.5 glass rounded-lg text-[9px] font-bold text-slate-400 light:text-slate-500 uppercase tracking-widest">{series.category}</span>
              ...
              <div className="space-y-8">
                ...
                <button 
                  ...
                  className="w-full py-4 xl:py-5 glass border border-white/10 light:border-slate-200 text-white light:text-slate-900 rounded-2xl font-bold transition-all flex items-center justify-center gap-4 text-[10px] xl:text-xs tracking-widest uppercase font-orbitron hover:bg-white/5 light:hover:bg-slate-100 active:scale-95"
                >
      ...
      {/* Floating Neural Pathway Modal - NO PUSH CONTENT */}
      <aside 
        className={`fixed top-20 right-0 bottom-0 z-[120] w-full sm:w-[380px] xl:w-[450px] bg-[#0a0c12]/98 light:bg-white/98 backdrop-blur-3xl border-l border-white/10 light:border-slate-200 shadow-[0_0_50px_rgba(0,0,0,0.8)] light:shadow-2xl transition-transform duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)] flex flex-col ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="p-6 xl:p-8 border-b border-white/10 light:border-slate-200 flex items-center justify-between bg-black/40 light:bg-slate-50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xs xl:text-sm font-bold font-orbitron text-white light:text-slate-900 tracking-[0.2em] uppercase">Vectra Map</h2>
              <span className="text-[8px] xl:text-[9px] text-slate-500 font-mono tracking-[0.4em] uppercase mt-1 block">Path Config v4.2</span>
            </div>
          </div>
          <button onClick={() => setIsSidebarOpen(false)} className="p-2.5 text-slate-500 hover:text-white light:hover:text-slate-900 transition-colors bg-white/5 light:bg-slate-200 rounded-full">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-hidden relative group">
          ...
          <div className="absolute bottom-6 xl:bottom-10 left-6 xl:left-10 right-6 xl:right-10 z-10 flex flex-col gap-5 pointer-events-none">
             <div className="grid grid-cols-2 gap-4">
               <div className="flex items-center justify-center gap-2.5 xl:gap-3 py-3 glass rounded-xl border border-white/5 light:border-slate-200 bg-black/60 light:bg-white/80">
                 <div className="w-2.5 h-2.5 bg-violet-600 rounded-full shadow-[0_0_10px_#8b5cf6]" />
                 <span className="text-[8px] xl:text-[9px] font-bold text-slate-400 light:text-slate-600 uppercase tracking-widest font-mono">Synced</span>
               </div>
               <div className="flex items-center justify-center gap-2.5 xl:gap-3 py-3 glass rounded-xl border border-white/5 light:border-slate-200 bg-black/60 light:bg-white/80">
                 <div className="w-2.5 h-2.5 bg-cyan-500 rounded-full animate-pulse shadow-[0_0_10px_#06b6d4]" />
                 <span className="text-[8px] xl:text-[9px] font-bold text-slate-400 light:text-slate-600 uppercase tracking-widest font-mono">Active</span>
               </div>
            </div>
          </div>
        </div>

        <div className="p-6 xl:p-8 bg-black/80 light:bg-slate-50 border-t border-white/10 light:border-slate-200">
      ...
      {/* Backdrop for accessibility and click-to-close */}
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-black/40 light:bg-slate-900/10 backdrop-blur-[2px] z-[110] animate-reveal"
